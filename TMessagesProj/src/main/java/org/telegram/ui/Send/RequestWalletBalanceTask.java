...
public final class RequestWalletBalanceTask {
    ...
    public void requestWalletBalance(final Address... addresses) {
        /*backgroundHandler.post(new Runnable()
		{
			...
			public void run()
			{
				final StringBuilder url = new StringBuilder(*.BITEASY_API_URL);
				...
				try
				{
					...
					if (responseCode == HttpURLConnection.HTTP_OK)
					{
						...
						for (int i = 0; i < jsonOutputs.length(); i++)
						{
							...
							if (tx == null)
							{
								tx = new FakeTransaction(Network.NETWORK_PARAMETERS, uxtoHash);
							...
							while (tx.getOutputs().size() < uxtoIndex)
								tx.addOutput(new TransactionOutput(Network.NETWORK_PARAMETERS, tx, Coin.NEGATIVE_SATOSHI, new byte[] {}));

							// add the real output
							final TransactionOutput output = new TransactionOutput(Network.NETWORK_PARAMETERS, tx, uxtoValue, uxtoScriptBytes);
							tx.addOutput(output);
						}
