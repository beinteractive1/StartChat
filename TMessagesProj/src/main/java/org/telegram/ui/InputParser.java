...
public abstract class InputParser
{
	...
	public abstract static class StringInputParser extends InputParser
	{
		...
		public void parse()
		{
			...
			else if (input.startsWith("bitcoin:"))
			{
				try
				{
					...
					if (address != null && !Network.NETWORK_PARAMETERS.equals(address.getParameters()))
			...
			else if (PATTERN_BITCOIN_ADDRESS.matcher(input).matches())
			{
				try
				{
					final Address address = new Address(Network.NETWORK_PARAMETERS, input);
					...
					|| PATTERN_DUMPED_PRIVATE_KEY_COMPRESSED.matcher(input).matches())
			{
				try
				{
					final VersionedChecksummedBytes key = new DumpedPrivateKey(Network.NETWORK_PARAMETERS, input);
			...
			else if (PATTERN_BIP38_PRIVATE_KEY.matcher(input).matches())
			{
				try
				{
					final VersionedChecksummedBytes key = new BIP38PrivateKey(Network.NETWORK_PARAMETERS, input);
			...
			else if (PATTERN_TRANSACTION.matcher(input).matches())
			{
				try
				{
					final Transaction tx = new Transaction(Network.NETWORK_PARAMETERS, Qr.decodeDecompressBinary(input));
		...
		{
			final Address address = new Address(Network.NETWORK_PARAMETERS, ((DumpedPrivateKey) key).getKey().getPubKeyHash());
	...
	public abstract static class BinaryInputParser extends InputParser
	{
		...
		public void parse()
		{
			if (Network.MIMETYPE_TRANSACTION.equals(inputType))
			{
				try
				{
					final Transaction tx = new Transaction(Network.NETWORK_PARAMETERS, input);
	...
	public static PaymentIntent parsePaymentRequest(@Nonnull final byte[] serializedPaymentRequest) throws PaymentProtocolException
	{
		try
		{
			...
			if (!paymentSession.getNetworkParameters().equals(Network.NETWORK_PARAMETERS))
	...
	private static final Pattern PATTERN_DUMPED_PRIVATE_KEY_UNCOMPRESSED = Pattern.compile((Network.NETWORK_PARAMETERS.getId().equals(
            NetworkParameters.ID_MAINNET) ? "5" : "9")
            + "[" + new String(Base58.ALPHABET) + "]{50}");
	private static final Pattern PATTERN_DUMPED_PRIVATE_KEY_COMPRESSED = Pattern.compile((Network.NETWORK_PARAMETERS.getId().equals(
