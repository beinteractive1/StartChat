...
public class WalletUtils
{
    ...
    {
        return formatHash(prefix, address.toString(), groupSize, lineSize, CurrencyCharConstants.CHAR_THIN_SPACE.value());
    }

    public static Editable formatHash(@Nonnull final String address, final int groupSize, final int lineSize)
    {
        return formatHash(null, address, groupSize, lineSize, CurrencyCharConstants.CHAR_THIN_SPACE.value());
    }
        ...
        for (final TransactionOutput output : tx.getOutputs())
        {
            try
            {
                if (!output.isMine(wallet))
                {
                    ...
                    return script.getToAddress(Network.NETWORK_PARAMETERS, true);
                }
            }
    ...
    public static Wallet restorePrivateKeysFromBase58(final InputStream is) throws IOException
    {
        ...
        final KeyChainGroup group = new KeyChainGroup(Network.NETWORK_PARAMETERS);
        group.importKeys(WalletUtils.readKeys(keyReader));
        return new Wallet(Network.NETWORK_PARAMETERS, group);
    }
    ...
    public static void writeKeys(@Nonnull final Writer out, @Nonnull final List<ECKey> keys) throws IOException
    {
        ...
        {
            out.write(key.getPrivateKeyEncoded(Network.NETWORK_PARAMETERS).toString());
    ...
    public static List<ECKey> readKeys(@Nonnull final BufferedReader in) throws IOException
    {
        try
        {
            ...
            while (true)
            {
                ...
                if (charCount > BACKUP_MAX_CHARS)
                    throw new IOException("read more than the limit of " + BACKUP_MAX_CHARS + " characters");
                if (line.trim().isEmpty() || line.charAt(0) == '#')
                    continue; // skip comment

                final String[] parts = line.split(" ");

                final ECKey key = new DumpedPrivateKey(Network.NETWORK_PARAMETERS, parts[0]).getKey();
