			...
			while (running.get())
			{
				...
				try
				{
					...
					for (int i = 0; i < numMessages; i++)
					{
						...
						try
						{
							final Transaction tx = new Transaction(Network.NETWORK_PARAMETERS, msg);
		...
		public void run()
		{
			while (running.get())
			{
				...
				try
				{
					...
					for (final Transaction tx : PaymentProtocol.parseTransactionsFromPaymentMessage(Network.NETWORK_PARAMETERS, payment))
					{
						if (!handleTx(tx))
							ack = false;
					}
