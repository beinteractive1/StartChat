...
import org.telegram.bitcoin.providers.AddressBookProvider;
import org.telegram.bitcoin.providers.WalletBalanceWidgetProvider;
import org.telegram.messenger.ApplicationLoader;
import org.telegram.messenger.R;
import org.telegram.ui.WalletActivity;
import org.telegram.utils.CrashReporter;
import org.telegram.utils.ThrottlingWalletChangeListener;
...
public class BlockChainServiceImpl extends Service implements BlockChainService
{
            ...
            handler.post(new Runnable()
            {
                ...
                public void run()
                {
                    ...
                    else
                    {
                        ...
                        notification.setContentTitle(getString(R.string.AppName));
    ...
    private final BroadcastReceiver connectivityReceiver = new BroadcastReceiver()
    {
        ...
        private void check()
        {
            ...
            if (impediments.isEmpty() && peerGroup == null)
            {
                ...
                peerGroup = new PeerGroup(Network.NETWORK_PARAMETERS, blockChain);
                peerGroup.setDownloadTxDependencies(false); // recursive implementation causes StackOverflowError
                peerGroup.addWallet(wallet);
                peerGroup.setUserAgent(Network.USER_AGENT, application.packageInfo().versionName);
                ...
                peerGroup.setConnectTimeoutMillis(TimeoutConstants.PEER_TIMEOUT_MS.timeout());

                peerGroup.addPeerDiscovery(new PeerDiscovery()
                {
                    private final PeerDiscovery normalPeerDiscovery = new DnsDiscovery(Network.NETWORK_PARAMETERS);
                    ...
                    public InetSocketAddress[] getPeers(final long timeoutValue, final TimeUnit timeoutUnit) throws PeerDiscoveryException
                    {
                        ...
                        if (hasTrustedPeer)
                        {
                            ...
                            final InetSocketAddress addr = new InetSocketAddress(trustedPeerHost, Network.NETWORK_PARAMETERS.getPort());
    ...
    public void onCreate()
    {
        ...
        config = application.getBitCoinConfiguration();
        final Wallet wallet = application.getWallet();
        ...
        blockChainFile = new File(getDir("blockstore", Context.MODE_PRIVATE), FileConstants.BLOCKCHAIN_FILENAME.reference());
        ...
        try
        {
            blockStore = new SPVBlockStore(Network.NETWORK_PARAMETERS, blockChainFile);
            ...
            if (!blockChainFileExists && earliestKeyCreationTime > 0)
            {
                try
                {
                    ...
                    final InputStream checkpointsInputStream = getAssets().open(FileConstants.CHECKPOINTS_FILENAME.reference());
                    CheckpointManager.checkpoint(Network.NETWORK_PARAMETERS, checkpointsInputStream, blockStore, earliestKeyCreationTime);
                    log.info("checkpoints loaded from '{}', took {}ms", FileConstants.CHECKPOINTS_FILENAME.reference(), System.currentTimeMillis() - start);
                }
        ...
        try
        {
            blockChain = new BlockChain(Network.NETWORK_PARAMETERS, wallet, blockStore);
        }
        catch (final BlockStoreException x)
        {
    ...
    public void onDestroy()
    {
        ...
        ApplicationLoader.scheduleStartBlockchainService(this);
